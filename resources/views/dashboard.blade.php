        ...
        <form id="todoForm" class="mb-6" action="{{ route('tasks.store') }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <div class="flex items-center border rounded-md px-4 py-2">
                    ...
                    <!-- Input Deadline -->
                    <input type="datetime-local" name="due_date" required class="ml-4 w-40 border-none focus:ring-0"
                        id="taskDeadline">
                </div>
            </div>
                ...
                fetch(url, {
                    ...
                    .then(response => {
                        if (response.ok) {
                            ...
                            if (isChecked) {
                                row.querySelector('.task-title').classList.add('line-through',
                                    'text-gray-500');
                                row.querySelector('.task-deadline').classList.add('text-gray-500');
                                taskTable.appendChild(row);
                            } else {
                                row.querySelector('.task-title').classList.remove('line-through',
                                    'text-gray-500');
                                row.querySelector('.task-deadline').classList.remove(
                                    'text-gray-500');
